...
### Подключение AI-анализа (необязательно)

Расчёт Score не требует сети или ключа. Чтобы включить кнопку «Получить AI-анализ», создайте ключ в OpenAI и задайте его **только локально**. Самый удобный способ — скопировать `backend/.env.example` в `backend/.env` и заполнить ключ:

```bash
cd backend
cp .env.example .env
# Откройте .env и укажите OPENAI_API_KEY=ваш_ключ
go run ./cmd/api
```

Файл `.env` исключён из Git и не отправляется браузеру. Запрос к OpenAI выполняется только после нажатия кнопки AI-анализа; модель получает готовый расчёт и формулирует объяснение, не пересчитывая Score.
